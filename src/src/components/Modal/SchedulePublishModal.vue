<template>
        ...
        <div v-if="show" 
          ...
          <Transition
            ...
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white"
                 @click.stop>
              <div class="mt-3">
                <h3 class="text-lg font-medium text-gray-900 mb-4">Отложенная публикация</h3>
                ...
                <form @submit.prevent="handleSubmit">
                  ...
                  <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <h4 class="font-medium text-blue-900 mb-2">Предварительный просмотр:</h4>
                    <div class="text-sm text-blue-800">
                      <p class="mb-2"><strong>Текст:</strong> {{ previewText }}</p>
                      <p v-if="post?.url"><strong>Ссылка:</strong> {{ post.url }}</p>
                      <p v-if="post?.media?.length"><strong>Медиафайлов:</strong> {{ post.media.length }}</p>
                      <p v-if="scheduledDateTime"><strong>Время публикации:</strong> {{ formattedDateTime }}</p>
                    </div>
                  </div>
                  ...
                  <div class="flex justify-end space-x-3">
                    ...
                    <button type="submit"
                            :disabled="!formData.channel || !formData.date || !formData.time || loadingChannels || submitting"
                            class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed">
                      {{ submitting ? 'Планирование...' : 'Запланировать' }}
                    </button>
                  </div>
                </form>
              </div>
  ...
  watch(() => props.show, (newShow) => {
    if (newShow) {
      ...
      formData.time = futureTime.toTimeString().split(' ')[0].substring(0, 5);
    }
  });
