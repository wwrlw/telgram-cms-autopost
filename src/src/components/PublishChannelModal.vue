  ...
  <div v-if="show" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 w-96 max-w-md">
      <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-semibold">Выберите канал для публикации</h3>
      ...
      <div v-else-if="error" class="text-center py-4">
        ...
        <button @click="loadChannels" class="mt-2 text-blue-600 hover:text-blue-800">
          Попробовать снова
        </button>
      </div>
      ...
      <div v-else>
        <div class="space-y-2 max-h-64 overflow-y-auto">
          <div
            ...
            :class="{ 'border-blue-500 bg-blue-50': selectedChannel?._id === channel._id }"
          >
        ...
        <div v-if="channels.length === 0" class="text-center py-4">
          <p class="text-gray-600">Нет доступных каналов для публикации</p>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async publish() {
      ...
      try {
        return new Promise((resolve, reject) => {
          http.publishToChannel(this.postId, this.selectedChannel.channel_id, (response) => {
            if (response.success) {
              this.$emit('published', {
                success: true,
