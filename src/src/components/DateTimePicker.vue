        ...
        <div class="space-y-2">
            ...
            <div class="flex flex-wrap gap-2">
                <button
                    ...
                    :class="{
                        'bg-blue-100 border-blue-300 text-blue-700': isQuickOptionSelected(option)
                    }"
                >
                    {{ option.label }}
                </button>
        ...
        <div v-if="selectedDateTime" class="p-3 bg-gray-50 rounded text-sm">
            <div class="flex items-center gap-2">
                ...
                <span class="text-gray-700">
                    Публикация: <strong>{{ formatDateTime(selectedDateTime) }}</strong>
                </span>
            </div>
        </div>
    </div>
...
function setQuickOption(option) {
    ...
    if (option.custom) {
        ...
        targetDate = new Date(now.getTime() + option.days * 24 * 60 * 60 * 1000);
    }
...
function isQuickOptionSelected(option) {
    ...
    if (option.custom) {
        ...
        const expected = new Date(now.getTime() + option.hours * 60 * 60 * 1000);
        return Math.abs(selected.getTime() - expected.getTime()) < 60000;
    } else if (option.days) {
        const expected = new Date(now.getTime() + option.days * 24 * 60 * 60 * 1000);
        return Math.abs(selected.getTime() - expected.getTime()) < 60000;
    }
