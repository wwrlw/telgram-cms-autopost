  ...
  <div class="bg-white shadow overflow-hidden sm:rounded-md">
    <div class="px-4 py-5 sm:p-6">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg leading-6 font-medium text-gray-900">Категории ({{ categories.length }})</h3>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center space-x-2">
                  <div class="h-6 w-6 rounded border border-gray-300" 
                       :style="{ backgroundColor: category.color || '#3B82F6' }"></div>
                  <span class="text-sm text-gray-500">{{ category.color || '#3B82F6' }}</span>
                </div>
              </td>
      ...
      <div v-if="categories.length === 0 && !loading" class="text-center py-12">
        ...
        <h3 class="mt-2 text-sm font-medium text-gray-900">Нет категорий</h3>
        <p class="mt-1 text-sm text-gray-500">Начните с создания первой категории для организации контента.</p>
        <div class="mt-6">
          <button @click="$emit('create-category')"
...
const emit = defineEmits(['update:selectedCategories', 'edit', 'delete', 'create-category'])
...
const toggleSelectAll = () => {
  if (selectAll.value) {
    selectedCategories.value = props.categories.map(category => category.id)
  } else {
...
watch(selectedCategories, () => {
  selectAll.value = selectedCategories.value.length === props.categories.length && props.categories.length > 0
})
</script> 