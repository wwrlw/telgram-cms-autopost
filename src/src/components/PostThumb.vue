<template>
  <div class="group bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-200">
    ...
    <div class="p-4">
      <router-link :to="{ name: 'post', params: { id: post._id } }" class="font-medium text-gray-900 mb-2 line-clamp-2 cursor-pointer">
        {{ extractTitle(post.text) }}
      </router-link>

      <router-link :to="{ name: 'post', params: { id: post._id } }" class="text-sm text-gray-600 mb-3 line-clamp-3 cursor-pointer">
        {{ post.text }}
      </router-link>
