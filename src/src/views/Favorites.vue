<template>
    <div class="min-h-screen bg-gray-50">
      <main class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div v-else-if="favoritePosts.length === 0" class="text-center py-12">
...
const canGoPrevious = computed(() => {
  if (!favoritePosts.value[currentMediaIndex.value]?.media?.length || !currentMedia.value) return false
  const mediaArr = favoritePosts.value[currentMediaIndex.value].media
  const currentIdx = mediaArr.findIndex(m => m.file_path === currentMedia.value.file_path)
  return currentIdx > 0
})
...
const deletePost = (postId) => {
  showConfirm('Вы уверены, что хотите удалить этот пост?', (id) => {
    http.deletePost({ id }, (response) => {
      if (response.success) {
        favoritePosts.value = favoritePosts.value.filter(post => post._id !== id)
        ...
        if (window._notify) {
          window._notify('error', 'Ошибка при удалении поста')
        }
      }
    })
  }, postId)
