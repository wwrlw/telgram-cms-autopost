  ...
  return new Promise((resolve) => {
    http.getFavoritePosts(userId.value, (response) => {
      if (response.success) {
        ...
        console.error('Failed to load favorite posts:', response.message)
      }
  ...
  return new Promise((resolve) => {
    http.addFavoritePost(userId.value, postId, (response) => {
      if (response.success) {
        ...
        if (window._notify) {
          window._notify('error', 'Ошибка при добавлении в избранное')
        }
        resolve(false)
      }
    })
...
const removeFromFavorites = async (postId) => {
  ...
  return new Promise((resolve) => {
    http.removeFavoritePost(userId.value, postId, (response) => {
      if (response.success) {
        favoritePosts.value = favoritePosts.value.filter(id => id !== postId)
        if (window._notify) {
        ...
        if (window._notify) {
          window._notify('error', 'Ошибка при удалении из избранного')
        }
        resolve(false)
      }
    })
